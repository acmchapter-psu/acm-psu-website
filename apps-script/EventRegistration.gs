...
var REGISTRATION_EVENTS = {
  jam26: {
    ...
    uniqueChecks: [
      {
        headers: ["University Email"],
        fields: ["universityEmail"],
        message: "this participant is already registered",
      },
      {
        headers: ["University ID"],
        fields: ["universityId"],
        message: "this participant is already registered",
      },
    ],
...
function doPost(e) {
  ...
  var reply = function (message) {
    return registrationReply(event, message, requestId);
  };
  if (!Object.prototype.hasOwnProperty.call(REGISTRATION_EVENTS, event))
    return reply("Error: unsupported event");
  if (requestId && !/^[a-f0-9]{32}$/.test(requestId))
    return registrationReply(event, "Error: invalid request identifier", "");
  if (e && e.postData && e.postData.length > 20000)
    return reply("Error: submission is too large");
  if (String(form.website || "").trim())
    return reply("Error: registration could not be accepted");
  ...
  try {
    ...
    if (count >= 120)
      return reply("Error: registration is busy. Please try again in a minute");
    ...
    return reply(
      "Error: registration could not be confirmed. Contact the organizers before retrying.",
    );
  } finally {
    try {
      lock.releaseLock();
...
function handleEventRegistration(config, form) {
  ...
  var value = function (field) {
    return String(form[field] === undefined ? "" : form[field]).trim();
  };
  ...
  for (i = 0; i < config.required.length; i += 1) {
    if (!value(config.required[i]))
      return reply("Error: missing " + config.required[i]);
  }

  // Optional block is all-or-nothing.
  var group = config.optionalGroup || [];
  var filled = group.filter(function (field) {
    return value(field);
  });
  var hasGroup = group.length > 0 && filled.length === group.length;
  if (filled.length && !hasGroup) {
    return reply(
      "Error: complete every optional member field or leave them all blank",
    );
  }
  ...
  for (i = 0; i < fieldNames.length; i += 1) {
    ...
    if (value(field).length > limit)
      return reply("Error: " + field + " is too long");
  }
  ...
  if (hasGroup)
    emailFields = emailFields.concat(config.optionalGroupEmails || []);
  ...
  if (config.sheet === "jam26" && value("teamMembers")) {
    var teammates = value("teamMembers")
      .split(",")
      .map(function (email) {
        return email.trim();
      });
    if (
      teammates.length > 10 ||
      teammates.some(function (email) {
        return !isRegistrationEmail(email) || email.length > 254;
      })
    ) {
      return reply(
        "Error: enter up to 10 valid team member emails separated by commas",
      );
    }
  }
  ...
  if (!sheet)
    return reply(
      "Error: registration storage is not ready. Please contact the organizers.",
    );
  ...
  if (
    identity &&
    !allowRegistrationRequest(config.sheet, identity, REGISTRATION_RATE_SECONDS)
  ) {
...
function mirrorRegistrationToPlatform_(eventKey, headers, row, requestId) {
  ...
  var token = String(
    properties.getProperty("PLATFORM_INTAKE_TOKEN") || "",
  ).trim();
  if (!url || !token) return;
...
function registrationSheet_(name) {
  if (!Object.prototype.hasOwnProperty.call(REGISTRATION_EVENTS, name))
    return null;
  return SpreadsheetApp.openById(REGISTRATION_SPREADSHEET_ID).getSheetByName(
    name,
  );
}
  ...
  for (var c = 0; c < checks.length; c += 1) {
    ...
    for (var h = 0; h < check.headers.length; h += 1) {
      ...
      for (var r = 0; r < rows.length; r += 1) {
        var cell = String(rows[r][col] === undefined ? "" : rows[r][col])
          .trim()
          .toLowerCase();
...
function registrationReply(event, message, requestId) {
  ...
  return HtmlService.createHtmlOutput(
    "<p>" +
      escapeRegistrationHtml(String(message)) +
      "</p><script>" +
      relay +
      "<\/script>",
